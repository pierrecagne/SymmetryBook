...
\begin{proof}[Elaboration on the previous proof, only (2)]
  ...
  First, let us emphasize that we are interested in
  the symmetries of $\blank^m$ {\em as a set bundle}, meaning we shall
  explore $\blank^m =_X \blank^m$ where $X$ is the type
  $\SetBundle(\Sc)$ (and not $\Sc\to \Sc$). Because $\SetBundle(\Sc)$
  is a subtype of $\sum_{A:\UU}A\to \Sc$, it is enough to determine
  the symmetries of $\blank^m$ in this later type. This is unfolded
  as:
  \begin{displaymath}
  ...
  to $(\Sc = C)$. Composing with univalence, we get that $(\Sc=\Sc)$
  is equivalent to $(\Sc \weq C)$. Following ((need a ref to
  exercise there)), one ends up with
  \begin{displaymath}
    D_m \weq \sum_{t:\Sc \weq C} \blank^mc =_{\Sc\to C} \blank^mt
  \end{displaymath}
  For simplicity, denote $F_m$ for the later type. Not too rigorously,
  the proof will go as follows:
  ...
  \item We shall describe the elements of $F_m$ as basically tuples
    ...
    and $q:\bn m\times Z = \bn m\times Y$ such that $q$ satisfies
    certain propositional equations.
  ...
  \item Finally we shall define a map $\myq: \bn m \to F_m$ properly
    ...
    equivalence. Picking an element $(Y,g,q)$ in the codomain, proving
    the fiber is contractible is a proposition, so that we can assume
    that $(Y,g)$ is equal to $(\zet,\etop s)$ (and not just merely
    equal). This element in $(Y,g)=(\zet,\etop s)$ allows us to craft
    an element in $(Y,g,q) = (\zet, \etop s, q')$ for some $q'$. This
    $q'$ is recognized through step 2 to be of the form $q_{i,n}$ for
    some $(i,n)$. Incidentally, it make $(Y,q,g)$ equal with $\myq i$
    providing a center of contraction $i$ in the fiber.
  \end{enumerate}

  {\sc Step 1.} An element of $F_m$ is given by a map $t:\Sc \to C$
  together with a term $!:\isEq(t)$ and a proof
  $Q: \blank^mt = \blank^mc$. Now such a $t$ can be reduced by
  $\Sc$-induction to the data of $t(\base)\defequi (Y,g,!)$ and
  $t(\Sloop)=(p,!,!): (Y,g,!) =_C (Y,g,!)$. Under identification by
  $\Sc$-induction, $\blank^mc$ is given by
  \begin{displaymath}
    \begin{split}
  ...
  \begin{displaymath}
    ...
    \end{split}
  \end{displaymath}
  By function extensionality and $\Sc$-induction again, $Q$ becomes
  then a term $q:m\times \zet = m\times Y$ such that the two following
  propositions hold:
  \begin{displaymath}
    {\sqrt[m]g} q = q \sqrt[m]{\etop s}%
    \quad\text{and}\quad
  ...
  $z\in\zet$
  \begin{displaymath}
    \ptoe q (j,z) = \ptoe q ( \sqrt[m]s^{j+nm} (0,0))%
    = \sqrt[m]g^{j+zm} \ptoe q(0,0)%
  \end{displaymath}
  Remark also for future reference that the proposition $p=g$ holds:
  indeed,
  \begin{displaymath}
    \id \times p = q \circ (\id \times {\etop s}) \circ q^{-1}
    = (q\sqrt[m]{\etop s}q^{-1})^m = (\sqrt[m]g)^n = \id \times g.
  \end{displaymath}
  ...
  pseudo inverse. Moreover direction computations show easily that the
  ...
  $(\zet,\etop s,!)$ together with $q_{i,n}$ yields (through
  $\Sc$-induction) an element $(c,!,Q_{i,n})$ of $F_m$.

  {\sc Step 3.} Let us then define $\myq: \bn m \to F_m$ by mapping
  $i \mapsto (c,!,Q_{i,0})$. The claim is that $\myq$ is an
  equivalence. Let $(t,!,Q)$ be an element of $F_m$ and as before
  represent $t$ by $\Sc$-induction:
  \begin{displaymath}
    \begin{split}
