...
\begin{remark}
  From now on, we shall forgo some rigour and use equalities between
  types and equivalences alike, going back and forth the family of
  maps $\cast_{X,Y}$ implicitly. What we loose in mathematical rigour,
  we gain it back in readability and ease of usage. To be a little
  more precise, given an equivalence $f:A\to B$, we shall freely use
  $f:A=B$ instead of the more rigorous $\etop f$. And conversely,
  given a path $p:A=B$, we shall feel free to write terms as $p(a):B$
  for some $a:A$ instead of the more rigourous $\ptoe p (a)$. Of
  course, we must restrain this convention to contexts where such $f$
  and $p$ only matters up to propositional equality. For any situation
  where such $f$ and $p$ need tobe defined up to definitional
  equality, we shall go back to the fully rigourous setting.
\end{remark}
